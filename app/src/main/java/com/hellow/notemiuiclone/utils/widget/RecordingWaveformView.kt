...
class RecordingWaveformView @JvmOverloads constructor(
    ...
    private val DEFAULT_PIXEL_PER_SECOND =
        AndroidUtils.dpToPx(ConstantValues.SHORT_RECORD_DP_PER_SECOND)
    ...
    private fun drawGrid(canvas: Canvas) {
        ...
        val end = Math.ceil(pxToMill(viewWidthPx) / gridStepMills).toInt()
        ...
        for (index in -1 until end + 1) {
            ...
            if (showTimeline) {
                ...
                //Draw timeline texts
                if (timeMills >= 0) {
                    val text = ConstantValues.formatTimeIntervalMinSec(timeMills)
